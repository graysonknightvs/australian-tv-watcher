...
IGNORE_WORDS = [
    "login", "sign-in", "privacy", "terms", "contact",
    "facebook", "instagram", "youtube", "twitter",
    "search", "subscribe", "accessibility"
]
...
def scrape_source(network, url):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if any(word in lower_url for word in IGNORE_WORDS):
            continue

        if network == "ABC" and "abc.net.au" not in lower_url:
            continue

        if network == "SBS" and "sbs.com.au" not in lower_url:
            continue

        if network == "10" and "10.com.au" not in lower_url:
            continue
...
def scrape_all():
    ...
    for network, urls in SOURCES.items():
        ...
        for url in urls:
            ...
            try:
                items = scrape_source(network, url)
...
def home():
    ...
    <p><a href="/api/scan">Run a scan</a></p>
    <p><a href="/api/programs">View saved programs</a></p>
    """
